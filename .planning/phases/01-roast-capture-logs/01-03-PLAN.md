...
must_haves:
  truths:
    ...
    - "Saving a roast persists notes, roast level, weights, and loss percent"
...
<task type="auto">
  ...
  <action>
    Add a pre-roast form for roast level (Light/Medium/Dark) and green weight grams, required before starting the timer. On Stop, transition to a post-roast form that collects roasted weight grams. Use Zod validation and show inline errors for missing or invalid weights. After post-roast entry, show a review screen summarizing times, weights, loss %, and notes (optional free-text). Provide Save and Discard actions; Discard must require a second confirmation and reset session state back to initial timer-ready state. Keep the flow sequence: Stop ends cooling -> roasted weight -> review -> save/discard.
  </action>
...
<success_criteria>
- Roast logs save required fields and compute loss percent.
